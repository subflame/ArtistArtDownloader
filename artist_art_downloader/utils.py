...
# ---------------------------------------------------------------------------
# Bulk-mode folder name sanitization (3-stage pipeline from Phase 1 plan)
# ---------------------------------------------------------------------------

# Characters illegal in BOTH Windows and Linux paths (control chars + shell metas)
_ILLEGAL_PATH_CHARS = re.compile(r'[\x00-\x1f<>:"/\\|?*\x7f]')

# Maximum total path length (conservative, leaves room for "output/.../cover.jpg")
_MAX_TOTAL_PATH = 240


def sanitize_folder_name(name: str) -> str:
    """Sanitize a name for use as a folder name in bulk download mode.

    Three-stage pipeline:
    1. NFKC normalization (composes decomposed chars, e.g. e+\u0301 -> \u00e9)
    2. Remove only truly illegal path characters (preserves accents, CJK, emoji)
    3. Trim dots/spaces and collapse whitespace

    Does NOT strip accents -- "Beyonce\u0301" becomes "Beyonce\u0301" (composed),
    NOT "Beyonce". This preserves readability.

    Examples:
        "Beyonce\u0301" -> "Beyonce\u0301" (preserved)
        "AC/DC"          -> "ACDC" (slash removed)
        "\u6c49\u5b57"          -> "\u6c49\u5b57" (CJK preserved)
        "Song \U0001f3b5 Name" -> "Song \U0001f3b5 Name" (emoji preserved)
    """
    # Stage 1: NFKC normalization
    normalized = unicodedata.normalize("NFKC", name)

    # Stage 2: Remove illegal path characters only
    sanitized = _ILLEGAL_PATH_CHARS.sub("", normalized)

    # Stage 3: Trim and collapse
...
def truncate_path_parts(artist: str, album: str) -> tuple[str, str]:
    """Ensure artist + album folder names fit within MAX_TOTAL_PATH budget.

    Budget calculation:
        output/ + artist/ + album/ + cover.jpg = ~22 chars overhead
        So artist + album must share ~218 chars.

    Proportionally truncates: artist gets 40%, album gets 60%.
    """
    overhead = 22  # "output/" + "/" + "/" + "cover.jpg"
    available = _MAX_TOTAL_PATH - overhead

    safe_artist = sanitize_folder_name(artist)
    safe_album = sanitize_folder_name(album)

    needed = len(safe_artist) + 1 + len(safe_album)
    if needed <= available:
        return safe_artist, safe_album

    # Proportional truncation
    artist_budget = int(available * 0.4)
    album_budget = available - artist_budget - 1

    if len(safe_artist) > artist_budget:
        safe_artist = safe_artist[:artist_budget].rstrip(". ")
    if len(safe_album) > album_budget:
        safe_album = safe_album[:album_budget].rstrip(". ")
...
def safe_join(root: Path, *parts: str) -> Path:
    ...
    After joining, resolves to absolute path and verifies it stays
    ...
    try:
        ...
        raise ValueError(
            f"Path traversal detected: resolved {resolved} "
            f"escapes root {root_resolved}"
        )
