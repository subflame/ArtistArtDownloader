...
COMPILATION_ALBUMARTISTS = {
    ...
    "varios artistas",
    "verschiedene künstler", "verschiedene interpret(en)",
    "artistes variés",
    "artisti vari",
    "artistas varios",
}
...
def _strip_collaboration_markers(artist: str) -> str:
    ...
    # (?:^|[\s\(\[\{]) consumes any leading whitespace/bracket before keyword
    # so "(feat. Guest)" doesn't leave "(" behind.
    # Trailing [\s\.:\(\[\{/] ensures we don't match words like "feather"
    artist = re.sub(
        r'(?:^|[\s\(\[\{])(feat|ft|f|featuring|vs|with|w|presents?|prod|and)\b[\s\.:\(\[\{/].*$',
        '',
        artist,
        flags=re.IGNORECASE,
    ).strip()
...
def _parse_artist_from_filename(file_path: Path) -> Optional[str]:
    ...
    artist = _strip_collaboration_markers(artist)
...
def find_similar_artists(artists: dict[str, ArtistContext], cutoff: float = 0.7) -> list[list[str]]:
    """Find groups of artist names that are similar enough to be the same artist.

    Uses difflib SequenceMatcher to detect close matches (e.g. "Big Pun"
    and "Big Punisher"). Each artist name appears in at most one group.

    Args:
        artists: Artist dict from scan_folder.
        cutoff: Similarity threshold 0.0-1.0. Default 0.7 works well for
                detecting abbreviations and minor variations.
    ...
    for name in names:
        if name in assigned:
            continue
        matches = difflib.get_close_matches(name, names, n=len(names), cutoff=cutoff)
        similar = [m for m in matches if m != name]
        if similar:
            group = [name] + similar
            groups.append(group)
            for g in group:
